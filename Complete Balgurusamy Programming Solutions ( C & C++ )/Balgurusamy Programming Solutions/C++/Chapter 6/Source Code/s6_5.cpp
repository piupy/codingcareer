...
void books :: input(int counter)
{
  ...
  char inpublisher[50];
  int instock;
 cout<<endl<<endl<<endl<<endl<<endl<<"Enter The Name of Author of Book "<<counter+1<<" :  ";
 gets(inauthor);
...
{
 float books :: *priceptr=& books :: price;     // Pointer To The Price Member
 int books :: *stockptr=& books :: stock;       // Pointer To The Stock Member
 ...
 this->*priceptr=pri;                        // Assigning The Values to Data Members //
...
void books :: output(int counter)
{
 ...
 cout<<endl<<endl<<"Price of Book "<<counter+1<<" :  "<<this->*priceptr;
 cout<<endl<<endl<<"Name of Publisher of Book "<<counter+1<<" :  "<<this->publisher;
 cout<<endl<<endl<<"No. of Copies of Book "<<counter+1<<" Available in Stock :  "<<this->*stockptr<<endl<<endl;
}
...
books *find(books *b,int s)
{
 ...
 for(int i=0;i<s;i++)
 {
	if(  ( strcmp( (b->title),utitle ) == 0 ) && ( strcmp( (b->author),uauthor ) == 0 ) )
	{
		found++;
		line();
		cout<<endl<<"1 Result Found ("<<(b->*stockptr)<<" Copies Available in Stock ) "<<endl;    // Prints the Copies of Found Book //
		details(b);
		break;
	}
	b++;
...
void details(books *b)
{
 ...
 cout<<endl<<endl<<"Price of Book :  "<<(b->*priceptr);
 cout<<endl<<endl<<"Name of Publisher of Book :  "<<(b->publisher);
 cout<<endl<<endl<<"No. of Copies of Book Available in Stock :  "<<(b->*stockptr)<<endl<<endl;
 books :: succ_tran++;
}
...
void priceupdate(books *b)
{
 ...
 for(int i=0;i< ::size;i++)
 {
	...
	{
		found++;
		cout<<endl<<"The Original Price of This Book :  "<<b->*priceptr;
		cout<<endl<<endl<<"Enter The New Price of The Book :  ";
		float new_price;
		cin>>new_price;
		b->*priceptr=new_price;
