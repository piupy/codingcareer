...
Partial Class _Default
    ...
    Protected Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        If ViewState("Action_On_Click") = "Insert" Then         ''''''''''''' Data Insertion
            ...
            cmd = New SqlCommand("usp_insert_tblgrid", con)
        ...
        ElseIf ViewState("Action_On_Click") = "Update" Then                       ''''' Data Updation
            con = New SqlConnection(CS)
            cmd = New SqlCommand("usp_update_tblgrid", con)
            cmd.CommandType = CommandType.StoredProcedure
    ...
    Protected Sub GetDataFromDatabase()
        ...
        con = New SqlConnection(CS)
        da = New SqlDataAdapter("select * from tblgrid order by id", con)
        ds = New DataSet()
        da.Fill(ds)
    ...
    Protected Sub GridView1_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles GridView1.RowCommand
        ...
        ElseIf e.CommandName = "updt" Then
            con = New SqlConnection(CS)
            da = New SqlDataAdapter("select name,gender,salary,country from tblgrid where id = " + e.CommandArgument.ToString(), con)
            ds = New DataSet()
            da.Fill(ds)
        ...
        ElseIf e.CommandName = "del" Then
            con = New SqlConnection(CS)
            cmd = New SqlCommand("delete from tblgrid where id = " + e.CommandArgument.ToString(), con)
            con.Open()
            cmd.ExecuteNonQuery()
            con.Close()
